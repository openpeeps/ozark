...
## This module implements the SQLite driver for Ozark, including the main
## database manager, connection pooling, and compile-time macros for
## finalizing and executing SQL queries. It also includes the
## logic for mapping query results to model instances when using SQLite as
## the database backend.
## 
## This module relies on the `db_sqlite` module for low-level database interactions,
## and the `sqlite_runtime` module for managing prepared statements and mapping results to models.

import std/[macros, macrocache, net, strutils,
            strformat, tables, locks, os]

import pkg/openparser/sql
import pkg/threading/[once, rwlock]
import pkg/db_connector/db_sqlite {.all.}
import pkg/db_connector/db_common {.all.}

import ./private/types
import ./dbmeta, ./sqlite_runtime
import ../model, ../query, ../collection

export SqlDriver, Port, strVal, `%`
export db_sqlite, model, query, sqlite_runtime, collection
...
type
  ...
  FinalizerProc* = proc() {.nimcall.}
  OzarkConnectionError* = object of CatchableError

var
  DB: ptr Ozark
  o = createOnce()
  stmtCacheLock*: RWLock
  stmtCache*: TableRef[DBConn, seq[FinalizerProc]]
...
proc initOzarkDatabase*(address: string) =
...
macro withDB*(body: untyped) =
  ...
  add result, quote do:
    block:
      ...
      let dbcon {.inject.} = open(db[].maindb.address, "", "", "")
      # defer:
      #   dbcon.close()
      block:
        `body`
...
proc initOzarkDbWithPool*(address: string, poolSize: Positive = 10) =
  ...
  initOzarkDatabase(address)
  initOzarkPool(poolSize)


proc initStmtCache*() =
  stmtCacheLock = createRwLock()
  if stmtCache.isNil:
    stmtCache = newTable[DBConn, seq[FinalizerProc]]()

proc registerStmt*(conn: DBConn, finalizer: FinalizerProc) =
  ## Register a finalizer for a prepared statement.
  ## Call this after preparing a statement:
  ##   let stmt = conn.prepare("...")
  ##   registerStmt(conn, proc () = stmt.finalize())
  writeWith stmtCacheLock:
    if not stmtCache.hasKey(conn):
      stmtCache[conn] = @[]
    stmtCache[conn].add(finalizer)

proc unregisterStmt*(conn: DBConn, finalizer: FinalizerProc) =
  ## Unregister a finalizer (call after finalizing a statement manually).
  writeWith stmtCacheLock:
    if stmtCache.hasKey(conn):
      var sseq = stmtCache[conn]
      var idx = -1
      for i, f in sseq:
        if f == finalizer:
          idx = i
...
proc finalizeAllPreparedStatements*(conn: DBConn) =
  ## Invoke and remove all registered finalizers for conn.
  var fmts: seq[FinalizerProc] = @[]
  writeWith stmtCacheLock:
    if stmtCache.hasKey(conn):
      fmts = stmtCache[conn]
      stmtCache.del(conn)
  ...
  for f in fmts:
    try:
      ...
      echo &"sqlite_runtime: error finalizing prepared statement: {getCurrentExceptionMsg()}"
...
proc closeOzarkPool*() =
  ...
  for c in db[].mainPool.connections:
    try:
      # Ensure runtime-level prepared statements are finalized for this connection
      # sqlite_runtime should provide a routine that finalizes/clears any cached
      # prepared statements tied to a DBConn.
      # Implement that proc in sqlite_runtime if it doesn't exist:
      #   proc finalizeAllPreparedStatements(conn: DBConn) =
      #     # iterate cache and finalize each stmt
      finalizeAllPreparedStatements(c)
      c.close()
    except DbError:
      # Try to finalize again (best-effort) and re-attempt close; if still failing,
      # swallow it after logging because we can't do much more at shutdown.
      try:
        finalizeAllPreparedStatements(c)
        c.close()
      except DbError:
        echo "Failed to fully close DBConn: unfinalized statements may remain"
  ...
  for c in db[].mainPool.busyConnections:
    ...
    try:
      ...
      try:
        ...
        echo "Failed to fully close DBConn (busy): unfinalized statements may remain"
...
proc acquireConn*(pool: DBConnectionPool, timeoutMs: int = 5000): DBConn =
  ...
  while waited <= timeoutMs:
    writeWith stmtCacheLock:
      ...
      # If pool can grow, open a new connection (best-effort)
      if pool.busyConnections.len < pool.maxSize:
        # use global DB credentials for main pool
        let gdb = getInstance()
        result = openConn(gdb[].maindb)
        pool.busyConnections.add(result)
        return
      sleep(stepMs)
      inc(waited, stepMs)
...
macro rawSQL*(models: ptr ModelsTable, sql: static string, values: varargs[untyped]): untyped =
  ...
  try:
    let sqlNode = parseSQL(sql, sqlDriver = SqlDriver.sqlite)
...
proc tryInsertIDCompat*(db: DbConn, query: SqlQuery, args: varargs[string, `$`]): int64 =
  ## Wrapper that handles both "INSERT" that yields SQLITE_DONE and
  ## "INSERT ... RETURNING" (which yields SQLITE_ROW), plus fallback to
  ## SELECT last_insert_rowid().
  try:
    let id = tryInsertID(db, query, args)
    if id >= 0: return id
  except:
    # fall through to fallbacks
    discard

  # If the statement returned a row (e.g. INSERT ... RETURNING id) try to read first column
  try:
    let v = db.getValue(query, args)
    if v.len > 0:
      try:
        return parseInt(v)
      except:
  ...
  # Final fallback: ask SQLite for last insert rowid on this connection
  try:
    let v = db.getValue(sql"SELECT last_insert_rowid()")
    if v.len > 0:
      try:
        return parseInt(v)
      except:
