...
block:
  ...
  for row in $4(dbcon, cols, SqlQuery("$1")):
    ...
    if isEmpty: continue
    if colNames.len == 0:
      colNames = cols.mapIt(it.name)
    var inst = new($2)
    for fName, fValue in inst[].fieldPairs():
      if fName in colNames:
        fValue = row[colNames.find(fName)]
      else:
        raise newException(OzarkModelDefect, "Model field `$2." & fName & "` does not have a corresponding column in the SQL result")
    results.entries.add(inst)
  results""" % [$parsedSql, $(getTypeImpl(m)[1]), assigns.join("\n    "), getRowProcName]
